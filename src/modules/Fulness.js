...
class Fulness {
    ...
    calcFulnessAfterExit() {
        if (JSON.parse(localStorage.getItem('gameLeftTime')) !== '0') {
            ...
            const diffTime =
                new Date().getTime() -
                JSON.parse(localStorage.getItem('gameLeftTime'));
            ...
            console.log(
                ...
                Math.floor(diffTime / TIMER_CONFIG.fulnessDecrease)
            );
            this.scene.fulness -= Math.floor(
                diffTime / TIMER_CONFIG.fulnessDecrease
            );
